...
class ConvertorPromptTemplate(StringPromptTemplate):
    ...
    def check_convertor_and_template(cls, values: Dict) -> Dict:
        print("VALIDATING IN CPT")
        # this is e.g. to ensure partialing knows what to do for ChatPromptTemplate
        values["input_variables"] = list(set(values["input_variables"]) | set(values["convertor_input_variables"]))
        return values

    def format(self, **kwargs: Any) -> str:
        kwargs = self._merge_partial_and_user_variables(**kwargs)

        convertor_kwargs = {k: v for k, v in kwargs.items() if k in self.convertor_input_variables}
        prompt_kwargs = {k: v for k, v in kwargs.items() if k not in self.convertor_input_variables}

        print(f"CALLING CONVERTOR {convertor_kwargs}")
        _converted = self.convertor(convertor_kwargs)
        # restrict to those which are featured in the prompt
        converted_kwargs = {
            k: _converted[k]
            for k in self.convertor_output_variables
        }

        full_kwargs = {**prompt_kwargs, **converted_kwargs}

        # Format the template with the input variables.
        return DEFAULT_FORMATTER_MAPPING[self.template_format](self.template, **full_kwargs)
